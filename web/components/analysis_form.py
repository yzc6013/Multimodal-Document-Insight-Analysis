...
def render_analysis_form(default_stock=None):
    """渲染股票分析表单"""
    ...
    # 关键修复：只要提供了default_stock，就更新缓存中的股票代码
    # 无论缓存中是否已有值，确保与最新选择同步
    if default_stock:
        # 只有当值不同时才更新，避免不必要的日志和状态变化
        if cached_config.get('stock_symbol') != default_stock:
            cached_config['stock_symbol'] = default_stock
            logger.info(f"📊 [股票代码更新] 从{(st.session_state.web_analysis_completed and '网页分析' or '文档分析')}结果更新股票代码: {default_stock}")
